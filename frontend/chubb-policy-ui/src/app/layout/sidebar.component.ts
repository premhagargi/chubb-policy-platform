...
@Component({
  ...
  template: `
    ...
    <aside
      class="fixed inset-y-0 left-0 z-50 flex w-[248px] flex-col transition-transform duration-200 lg:static lg:translate-x-0"
    ...
    >
      ...
      >
        ...
        >
        <span class="min-w-0">
          <span class="block text-[13px] font-semibold leading-tight tracking-tight text-white"
            >CHUBB</span
          >
          <span class="block text-[11px] leading-tight" style="color: var(--sidebar-text);"
            >Policy Platform</span
          >
        </span>
        <button
          type="button"
          class="ml-auto rounded p-1 text-white/70 hover:bg-white/10 lg:hidden"
          (click)="close.emit()"
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4" aria-label="Main">
          ...
          @for (item of primaryNav; track item.route) {
            <li>
              <a
                ...
                [style.color]="rla.isActive ? 'var(--sidebar-text-active)' : 'var(--sidebar-text)'"
                [style.background]="rla.isActive ? 'var(--sidebar-active)' : 'transparent'"
        ...
        <div class="my-4 h-px" style="background: var(--sidebar-border);"></div>
          ...
          @for (item of secondaryNav; track item.route) {
            <li>
              <a
                ...
                [style.color]="rla2.isActive ? 'var(--sidebar-text-active)' : 'var(--sidebar-text)'"
                [style.background]="rla2.isActive ? 'var(--sidebar-active)' : 'transparent'"
      ...
      <div class="p-3" style="border-top: 1px solid var(--sidebar-border);">
        <div class="flex items-center gap-2.5 rounded-md px-2 py-2">
          ...
          <span class="min-w-0 flex-1">
            <span class="block truncate text-[13px] font-medium text-white">Prem</span>
            <span class="block truncate text-[11px]" style="color: var(--sidebar-text);"
              >Administrator</span
            >
          </span>
        </div>
